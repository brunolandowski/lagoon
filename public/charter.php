...
?>	
	...
	<section >
        <div class="intro">
            <h1>Charters</h1>
            <h2>
                Long ago, the top charter companies recognised our catamarans as having the qualities that make them ideal for chartering: strength, reliability, comfort and seaworthiness.
            </h2>
            <div class="filters">
                <div class="drop-container">
                    <a class='btn' href="#" data-placeholder="Selectionnez un modèle" data-modelecount="{count} modèles selectionnés">Selectionnez un modèle</a>
                    <div class="dropbox">
                        <ul> 
                            <li data-id="380">380</li>
                            <li data-id="40">40</li>
                            <li data-id="42">42</li>
                            <li data-id="450S">450S</li>
                            <li data-id="450F">450F</li>
                            <li data-id="46">46</li>
                            <li data-id="50">50</li>
                            <li data-id="52S">52S</li>
                            <li data-id="52F">52F</li>
                            <li data-id="54">54</li>
                            <li data-id="560">560</li>
                            <li data-id="58">58</li>
                            <li data-id="620">620</li>
                            <li data-id="Seventy-7">Seventy 7</li>
                            <li data-id="630 MY">630 MY</li>
                            <li data-id="Seventy-8">Seventy 8</li>
                        </ul>
                    </div>
                </div>
                <div class="select">
        ...
        <div class="map">
            ...
            <div class="selected">
                ...
                <div class="text">
                    <div class="name">Vi-Dream Co.Ltd</div>
                    <div class="address">88/40 Moo 5, Chao Fah West Road<br>Chalong Sub-district, Muang Phuket District,<br>Phuket Province, 83130</div>
                    <div class="mail">info@sailshana.com</div>
                    <div class="tel"><b>Tel :</b> +65 9770 5568</div>
                    <div class="fax"><b>Fax :</b> +65 6324 0179</div>
                    <div class="boats"><b>Flotte :</b>  42 / 380 / Seventy 7</div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function showInfosCharter( marker ) {
            ...
            var scrollTop =  $('.map .selected').offset().top + $('.map').offset().top;
            $('body,html').animate({ scrollTop: scrollTop })
        }


        function filterBoats( list ) {
            
            currentBoatsList = list;
            var countryRestrictedList = [];
            var boatsRestrictedList = [];
            var finalRestrictedList = []; 

            if( $('#countries').val() != '' && countries.hasOwnProperty( $('#countries').val() ) ){
                countryRestrictedList = countries[ $('#countries').val() ];
            }        

            if( currentBoatsList.length > 0 ) {

                var ids = [];
                $(currentBoatsList).each( function( index, item){
                    ids = ids.concat( boats[item] );
                })

                $.each(ids, function(i, el){
                    if($.inArray(el, boatsRestrictedList) === -1) boatsRestrictedList.push(el);
                });
            }  
            if( countryRestrictedList.length > 0 ) {
                if( boatsRestrictedList.length > 0 ) {
                    for (var i = 0; i < boatsRestrictedList.length; i++) {
                        if( countryRestrictedList.indexOf( boatsRestrictedList[i] ) != -1 ) finalRestrictedList.push( boatsRestrictedList[i] );
                    }
                    finalRestrictedList.push( -1 ); // prevent empty array showing all results
                } else {
                    finalRestrictedList = countryRestrictedList;
                }
            } else {
                if( boatsRestrictedList.length > 0 ) {
                    finalRestrictedList = boatsRestrictedList;
                } else {
                    finalRestrictedList = [];
                }
            }
            ...
            for (var i = 0; i < markersList.length; i++) {
                ...
                if( finalRestrictedList.length > 0 ) {
                    if( finalRestrictedList.indexOf( marker.customInfo ) != -1 ) {
                        marker.setVisible(true);
                        finalMarkerList.push( marker );
            ...
            if( finalRestrictedList.length == 0 ) finalMarkerList = markersList;

            markerCluster.clearMarkers();
            markerCluster.addMarkers(finalMarkerList);
